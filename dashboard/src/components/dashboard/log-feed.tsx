...
export function LogFeed({ logs, className }: { logs: BotLogEvent[]; className?: string }) {
        ...
        {logs.length === 0 ? (
          ...
          [...logs].reverse().map((log, i) => (
            <div key={i} className={cn('flex gap-2 leading-5', levelStyle[log.level])}>
              ...
              <span className={cn('shrink-0 font-bold', levelStyle[log.level])}>{levelPrefix[log.level]}</span>
