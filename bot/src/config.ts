...
// Mutable config-object — bestaande module-referenties blijven geldig na reloadConfig()
export const config: BotConfig = buildConfig();

/**
 * Herlaad alle config-waarden vanuit de huidige process.env.
 * Aanroepen na update_config zodat gewijzigde settings zonder bot-herstart actief worden.
 */
export function reloadConfig(): void {
  const fresh = buildConfig();
  Object.assign(config.helius,  fresh.helius);
  Object.assign(config.wallet,  fresh.wallet);
  Object.assign(config.jupiter, fresh.jupiter);
  Object.assign(config.raydium, fresh.raydium);
  Object.assign(config.swap,    fresh.swap);
  Object.assign(config.tpsl,    fresh.tpsl);
  Object.assign(config.filter,  fresh.filter);
  config.dryRun   = fresh.dryRun;
